...
function UT.Spawn.setModeCivilians()
    ...
    if not UT.checkTable(UT.Spawn.available.civilians) then
        UT.showMessage("no civilians available here", UT.colors.red, "spawn")
        return
    end
...
function UT.Spawn.setModeLoots()
    ...
    if not UT.checkTable(UT.Spawn.available.loots) then
        UT.showMessage("no loots available here", UT.colors.red, "spawn")
        return
    end
...
function UT.Spawn.getPosition()
    if UT.Spawn.position == "crosshair" then
        ...
        if not crosshairRay then
            UT.showMessage("impossible to spawn", UT.colors.red, "spawn")
            return
        end
...
function UT.Spawn.setTeam(unit, team)
	...
	local teamId = tweak_data.levels:get_default_team_ID(team or "combatant")
	unit:movement():set_team(managers.groupai:state():team_data(teamId))
end
...
function UT.Spawn.removeNPCs()
    ...
    UT.showMessage("removed " .. tostring(count) .. " npc" .. (count > 1 and "s" or ""), UT.colors.green, "spawn")
end
...
function UT.Spawn.removeLoots()
    ...
    local count = UT.removeUnits(units)
    UT.showMessage("removed " .. tostring(count) .. " loot" .. (count > 1 and "s" or ""), UT.colors.green, "spawn")
end
...
function UT.Spawn.removeEquipments()
    ...
    local count = UT.removeUnits(units)
    UT.showMessage("removed " .. tostring(count) .. " equipment" .. (count > 1 and "s" or ""), UT.colors.green, "spawn")
end
...
function UT.Spawn.removePackages()
    ...
    local count = UT.removeUnits(units)
    UT.showMessage("removed " .. tostring(count) .. " package" .. (count > 1 and "s" or ""), UT.colors.green, "spawn")
end

function UT.Spawn.spawnDrivableCar()
    if UT.checkTable(managers.network:session():peers()) then
        UT.showMessage("driving mode is only available solo", UT.colors.red, "spawn")
        return
    end
    local id = "units/pd2_dlc_cage/vehicles/fps_vehicle_falcogini_1/fps_vehicle_falcogini_1"
    local vehiclePosition = UT.getPlayerPosition()
    local vehicleRotation = UT.getCameraRotationFlat()
    local playerPosition = Vector3(vehiclePosition[1], vehiclePosition[2], vehiclePosition[3] + 50)
    local playerRotation = UT.getCameraRotation()
    UT.spawnUnit(Idstring(id), vehiclePosition, vehicleRotation)
